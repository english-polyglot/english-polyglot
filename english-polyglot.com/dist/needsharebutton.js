...
(function () {
  ...
  window.needShareDropdown = function (elem, options) {
    ...
    var root = this;
    root.elem = elem;
    root.elem.className += root.elem.className.length
      ? " need-share-button"
      : "need-share-button";
    ...
    root.getTitle = function () {
      ...
      if (document.querySelector) {
        ...
        if ((content = document.title)) return content.innerText;
        else return "";
      }
    };
    ...
    root.share = {
      mailto: function () {
        var url =
          ...
          encodeURIComponent(root.options.url) +
          " - " +
          encodeURIComponent(root.options.description);
      ...
      linkedin: function () {
        var url =
          root.options.protocol + "www.linkedin.com/shareArticle?mini=true";
        url += "&url=" + encodeURIComponent(root.options.url);
        url += "&title=" + encodeURIComponent(root.options.title);
      ...
      googlebookmarks: function () {
        var url =
          root.options.protocol + "www.google.com/bookmarks/mark?op=edit";
      ...
      odnoklassniki: function () {
        var url =
          root.options.protocol +
          "connect.ok.ru/dk?st.cmd=WidgetSharePreview&st.share";
        url += "Url=" + encodeURIComponent(root.options.url);
        url += "&title=" + encodeURIComponent(root.options.description);
    ...
    root.options = {
      ...
      buttonText: "Share",
      protocol:
        ["http", "https"].indexOf(window.location.href.split(":")[0]) === -1
          ? "https://"
          : "//",
    ...
    if (root.options.shareButtonClass) {
      for (var i = 0; i < root.elem.children.length; i++) {
        if (
          root.elem.children[i].className.match(root.options.shareButtonClass)
        )
    ...
    root.button.addEventListener("click", function (event) {
      ...
      if (!root.elem.className.match(/need-share-button-opened/)) {
        ...
        root.elem.className = root.elem.className.replace(
    ...
    if (root.options.iconStyle == "box" && root.options.boxForm == "horizontal")
      ...
      root.options.iconStyle == "box" &&
      root.options.boxForm == "vertical"
    )
    ...
    setTimeout(function () {
      switch (root.options.position) {
        ...
        case "topCenter":
          ...
          root.dropdown.style.marginLeft =
            -root.dropdown.offsetWidth / 2 + "px";
          break;
        case "middleLeft":
          root.dropdown.className += " need-share-button_dropdown-middle-left";
          root.dropdown.style.marginTop =
            -root.dropdown.offsetHeight / 2 + "px";
          break;
        case "middleRight":
    ...
    var iconClass =
      root.options.iconStyle == "default"
        ? "need-share-button_link need-share-button_"
        : "need-share-button_link-" +
          root.options.iconStyle +
          " need-share-button_link need-share-button_";
