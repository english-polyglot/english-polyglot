/*! Phrase generator v1.3 | Copyright 2012, 2016 Mr.Poligloto | www.english-polyglot.com/generator-fraz.html */
...
//  var x='<em id="stop_intro" class="arrow-r ui-icon-delete" onclick="disappear();appear()"></em>';
    var x='<em id="stop_intro" class="arrow bottom right" onclick="disappear();appear()"></em>';
  ...
  var ij3='<span id="ij3" class="intras"><span id="i3">&#9658;</span><span id="j3">'+x+'Таблицу глаголов можно спрятать/показать здесь</span></span>';
  ...
  var gamma=0;
	...
	if (adblocker) {
	  document.getElementById("page1").innerHTML='<div id="adbl"><div> &nbsp;</div><br /><div class="info adblstop">Please turn off the Adblock on this site and restart the page or browser</div><br /><div> &nbsp;</div></div>'
	}
...
function block(gen,name){
		...
		if(name.innerHTML){name=name.innerHTML.match(/<aa>(.*?)<\/aa>/g)}
...
function start_urok() {
			...
			else
			{
			 ...
			 if(i==0){checkCookie('intro');appear()}
...
function click1() {
		...
		{	
			...
			else
			{
			  ...
			  data_top = '<em>'+c1+'</em><span><a id="hierarhy" href="javascript: void(0)" onclick="playSound3(mesmid, currentVerb, message_top);if(i==2){disappear();appear()}">'+ message_top +'</a></span>';
		...
		{  
			if (first=="a")
			{
			  ...
			  data_bot = '<em>'+c3+'</em><span><a id="hierarhy" href="javascript: void(0)" onclick="playSound3(mesmid2, currentVerb, message_bottom);if(i==2){disappear();appear()}">'+ message_bottom +'</a></span>';
...
function preload_pap(pap) {
		...
		function processConcatenatedFile( dataSound )
		{
			...
			while( offset < ss.byteLength )
			{
			  ...
			  if (pap==currentVerb){document.getElementById("progress2").style.background = '#3C8C3E'};
		};
				...
				if(this.alpha == this.target){
					...
					if (!fl){
					  ...
					  switch (i) {
						  ...
						  case 2:document.getElementById('hierarhy').style.color=col;break;
