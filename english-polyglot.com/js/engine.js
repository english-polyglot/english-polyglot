...
//	var x_left='<em id="stop_intro" class="arrow arrow-left"></em>';
  ...
  var intro=['<span class="intro1"'+x_code+'">'+x_bottom+'Для начала работы, нажмите на стрелку внизу.</span>','<span class="intro1"'+x_code+'">'+x_bottom+'Переведите фразу в уме. Нажмите стрелку снова, чтобы проконтролировать ваш перевод.</span>','<span class="intro1"'+x_code+'">'+x_top+'Чтобы прослушать фразу еще раз, кликните по ней самой.</span>','<span class="intro1"'+x_code+'">'+x_bottom2+'Ниже отображается таблица глаголов. Спрятать/показать её можно кнопкой с крестиком.</span>','<span class="intro1"'+x_code+'">'+x_bottom+'Кнопкой под фразами можно выбрать глагол.</span>','<span class="intro1"'+x_code+'">'+x_top+'Другие настройки генератора фраз находятся в верхнем меню.</span>','<span class="intro1"'+x_code+'">'+x_top+'В верхнем меню находятся остальные инструкции по работе с генератором фраз.</span>'];

  var ij3='<span id="ij3"><span class="intro1" onclick="disappear();appear()">'+x_bottom2+'</em>Ниже отображается таблица глаголов. Спрятать/показать её можно кнопкой с крестиком.</span></span>';
  
  var tabl=[,['tablica-glagolov-petrova-image.gif','tablica-glagolov-petrova-image-black.gif','red-tablica-glagolov-petrova-image.gif','table-base.gif'],,['tablica-glagola-to-be-petrova-image.gif','black-tablica-glagola-to-be-petrova-image.gif','green-tablica-glagola-to-be-petrova-image.gif','table-be.gif'],['tablica-glagola-to-be-ing-image.gif','tablica-glagola-to-be-ing-image-black.gif','tablica-glagola-to-be-ing-image-green.gif','table-be.gif']]
  var tab_index=0;
...
//	
	...
	r(function(){
		...
		$("#dbl").bind("vclick",function(){if(show){if(text_visibility||!e_space){click1plus1()}else{click1()}}});//||!e_space
	...
	$(window).bind("orientationchange",function(event){
		  ...
		  else
		  {
			  if($(window).width()<=370||$(window).height()<=370){
				  ...
				  if (urok<3||urok>6){$("#selector").css({"display":"block"});}
			  }
		  }
		  if (can_orient&&can_orient_global) if (show) {click1plus1();};
	})
...
function check_mp3() {
			...
			context.decodeAudioData(audioFromString, function (buffer) { }, function (e) {	
				audio_name = 'fff.ogg';
				phrase_name = 'nnn-ogg';
			});
	}
...
//    console.log($.mobile.urlHistory.stack);
//});
		setTimeout(function(){if(location.hash!=""){window.location.href=window.location.href.split("?")[0]}},0)
...
function block(gen,name,loc){
		...
		if(typeof all_uroks_sounds=='undefined'){};//новый ппытк
		...
		if(name.innerHTML){name=name.childNodes[0].innerHTML} //name.innerHTML.match(/<aa>(.*?)<\/aa>/g)
		glob_name = name;
		
		if(!loc)setTimeout(function(){window.location.replace((''+window.location).split('#')[0]+'#page1?urok_num='+gen+'&urok_title='+encodeURIComponent(name))},0);

		function updateProgress( p ) {
			progress_line.style.width = Math.round( p ) + '%';
		};
		
		document.getElementById("nameLesson").innerHTML = gen+'.&nbsp;&nbsp;'+name;
		
		$(":jqmData(role='page')").attr("data-title", document.title);
		document.getElementById("verb_all").innerHTML = '';
		document.getElementById("verb_counter").innerHTML = '';
		document.getElementById("progress_line").style.background = 'rgb(120,120,120)';
		switch (gen) {
			case 1: 
			{			  
			...
			case 6: {
			   ...
			   r(function(){setMaxWidth('950px')});//вместо $(document).ready	
		...
		if (urok<5) {
			tID.innerHTML ='<img src="img/'+tabl[(urok==2)?urok-1:urok][tab_index]+'" />';
		...
		for (var i=1;i<9;i++)
		{
			if(i!=urok){document.getElementById("urok"+i).style.display="none"}
			else{document.getElementById("urok"+i).style.display="block"};
		}
...
function loadFile( papp, url, progressCallback, callback ) {
	   ...
	   var xhr = null;
		...
		if (xhr) {    
			...
			if ((url.split('.').pop()!='xml')) {arrbuf = true;}
...
function click1minus1()	{
	  ...
	  if (numberOfSelectedVerbs>1 & bac!=0){randNum [currentVerb]--;}
	   ...
	   if (trans)
	   {
		   if (message_top == '')
		   {
			   if (sound & sound_global)
			   {
					 if (first=="a")
					 {
						 document.getElementById('conteiner3').innerHTML = '<em>'+c2+'</em><span></span>';	
						 document.getElementById("midline").innerHTML = dinamic;				 
					 }
					 else			 
					 {
						 document.getElementById('conteiner2').innerHTML = '<em>'+c2+'</em><span></span>';	
						 document.getElementById('translation_audio').innerHTML = dinamic;
					 }
			   }
		   }
...
function preload_pap(pap) {
		...
		var requestPhrase = loadFile( pap, phrase, updateProgressLine, function(){
		...
		{
		  var requestAudio = loadFile( pap, audio, updateProgressLine, function(){
		...
		function processConcatenatedFile( dataSound )
		{
			...
			while( offset < ss.byteLength )
			{
			  ...
			  if (pap==currentVerb){document.getElementById("progress_line").style.background = '#3C8C3E'};
		};
...
function playSound3(soundf, pap, soundr) {
		...
		{	 
			 if (floaded[pap]){
				if(lengthVerbs==1){var SL='/'}else{var SL='/'+pap+'/'}	
				document.getElementById("dummy").innerHTML=
				'<audio id="my-audio">'+
				'<source src="audio/g-'+urok+SL+soundfile+'.mp3" type="audio/mpeg" />'+
				'<source src="audio/g-'+urok+SL+soundfile+'.ogg" type="audio/ogg" />'+
				'<embed hidden="true" autostart="true" loop="false" src="audio/g-'+urok+SL+soundfile+'.mp3" />'+
				'</audio>';
...
//					  }
		...
		document.getElementById("progress_line").style.background = type_connection;	
...
function start_timer(id) {
		...
		a.opacity=0.8;
		a.zIndex='10';
		id_timer=window.setTimeout(function(){fadeOut('dbl');a.zIndex='0';document.getElementById(id).innerHTML=''},5000);
	}	
...
function an_switch(){
		if(i==3){fadeEffect.init('ij3',0);i++;appear()}//отдельный id-блок!
		...
		if(!anons){
			...
			an_s.innerHTML='<em id="an_sw" class="arrow-r ui-icon-info" onclick="an_switch();return false;"></em>'+ij3;	
		}else{
			ax.style.display='block';
			an_s.innerHTML='<em id="an_sw" class="arrow-r ui-icon-delete" onclick="an_switch();return false;"></em>'+ij3;				
		}	
	}
...
var fadeEffect=function(){
		return{
			init:function(id, flag, target){
				...
				if(i!=3){var inn=document.getElementById('intro')}else{var inn=document.getElementById('ij3')}
				...
				this.elem.si = setInterval(function(){fadeEffect.tween(flag)}, 20);
			},
			tween:function(fl){
				if(this.alpha == this.target){
					...
					if (!fl){
					  ...
					  switch (i) {
						  case 0:
						  case 1:document.getElementById('forward').style.backgroundColor=col;break;
						  case 2:if(document.getElementById('hierarhy')){document.getElementById('hierarhy').style.color=col;break}
						  case 3:document.getElementById('an_sw').style.backgroundColor=col;break;
						  case 4:document.getElementById('verb-button').style.backgroundColor=col;break;
						  case 5:document.getElementById('a_pageGear').style.backgroundColor=col;break;
						  case 6:document.getElementById('a_pageInst').style.backgroundColor=col;break;			
					  }
					}
				}else{
...
function appear(){
	if(i<7){
		if(urok>2&&urok<7){if(i==4){i++}if(i==2){shift_int=10}else{shift_int=0}}
...
function fadeOut(id) {
		...
		s.opacity = 0.8;
		(function fade() {if((s.opacity-=.1)<.1) {s.opacity=0}else{setTimeout(fade,50)}})();
	}
	
function goBack(){
	if(!relo){setTimeout(function(){window.location.assign('generator-fraz.html')},0)}else{window.history.back()}
}