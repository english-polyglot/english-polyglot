...
var newwin;	
window.name='parent';
var win_test=win_img=win_url=0;
var last_url,last_width,last_height;
	
//function timer(callback){
//	console.log('timer')
//	if(newwin.closed) return
//	setTimeout(timer, 10)
//}
//var foo = 'poo';
//function timer(callback) {
...
//function r(f){/in/.test(newwin.closed)?setTimeout('r('+f+')',9):f()}
...
function flow(){
    ...
    if((newwin==undefined)||!(newwin && !newwin.closed)){	// если окно открывается впервые или уже после его закрытия, то...
		
		//console.log('новое newwin='+newwin)
		// ... открываем окно
				
		if (parent.screen.width<800){ // если наш гаджет это мобила или планш, то... 
			
			// проводим проверку на контент
			if(url.match(/htm/i)){ //если это отдельная страница(практика для урока генератора, фрагмент фильма или доп.инфа) 
				//win_url=1
				newwin=window.open(url,'newwin') // то сразу открываем эту страницу в новой пустой вкладке
			
			}else{ // если это не целая страница, а данные 
				// то открываем сначала новую пустую вкладку во всю ширину экрана
				newwin=window.open('answer-29-01-2017.html','newwin')
			}
			
		}else{ // если наш гаджет - десктоп или лэптоп, то сначала определяем размеры экрана
...
//			params+=',top='+top+',left='+left
		...
		bindReady(function(){ // ждем построения DOM 
			if(url==''){ //  если это вопрос теста, то после построения DOM вставляем блок ответа
				...
				if(url.match(/jpg|gif|png/i)){
					...
					//если это отдельная страница(практика для урока генератора, фрагмент фильма или доп.инфа), то 			
					//alert('hi')
					win_url=1
	...
	}else{ // Теперь. Ежели окно открывается повторно, то окно вовсе не открывается
	//console.log('повторное newwin='+newwin)	
	if(newwin) newwin.focus() // сначала выводим окно на первый план
		if(url==''){ // ... если это вопрос теста, то меняем блок ответа(заголовок окна, рамку!! и фразы) ...  
			...
			if(url.match(/jpg|gif|png/i)){ // если картинка, то меняем картинку
...
//	} //стоявший на этом месте else вызывал сбой в IE 
    ...
    if (window.addEventListener)
		...
		newwin.onload = function() { // replace by new onload and call the old one
			fn && fn()
			ready()
		}
    }
	...
	if(win_test||win_img||win_url){ // выясняем, это первый или повторный вход
		// если повторный, то ...
		if(last_url!=url){ // и если целиковый урл не совпадает 
		//console.log('url не совпадает...')
			if ((last_width!=width)||(last_height!=height)){ // и не совпадают размеры окна, то подобный вход - первый...
...
// нужна для закрытия дочерних окон, когда родительское окно уже закрыто 
...
function getextension(filename){
 return /[^?]*/.exec(/[^.]+$/.exec(filename))[0]} // оставляем всё, что после ".", а потом обрезаем всё что после "?" 
