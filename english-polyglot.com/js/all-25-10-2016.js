...
function popup(
  ...
  function flow() {
    ...
    if (newwin == undefined || !(newwin && !newwin.closed)) {
      ...
      if (parent.screen.width < 800) {
        ...
        if (url.match(/htm/i)) {
          //если это отдельная страница(практика для урока генератора, фрагмент фильма или доп.инфа)
          //win_url=1
      ...
      // Теперь. Ежели окно открывается повторно, то окно вовсе не открывается
      //console.log('повторное newwin='+newwin)
      if (newwin) newwin.focus(); // сначала выводим окно на первый план
      if (url == "") {
        // ... если это вопрос теста, то меняем блок ответа(заголовок окна, рамку!! и фразы) ...
  ...
  if (win_test || win_img || win_url) {
    // выясняем, это первый или повторный вход
    // если повторный, то ...
    if (last_url != url) {
      // и если целиковый урл не совпадает
      //console.log('url не совпадает...')
      if (last_width != width || last_height != height) {
        // и не совпадают размеры окна, то подобный вход - первый...
...
function getextension(filename) {
  return /[^?]*/.exec(/[^.]+$/.exec(filename))[0];
} // оставляем всё, что после ".", а потом обрезаем всё что после "?"
