...
$("#dbl").bind( "vclick", function() {alert('hi');if (show) {click1plus1();} });
...
$("#page1").on( "swipeleft", function() {
		...
		if (!text_visibility) {document.getElementById("text_monitor").innerHTML = 'Swipe&rarr;right to show text';clearTimeout(id_timer);start_timer("text_monitor")}
		else {document.getElementById("text_monitor").innerHTML = 'Swipe&rarr;right to show text';start_timer("text_monitor")};	
	});
...
$("#page1").on( "swiperight", function() {
		...
		if (!text_visibility) {
			document.getElementById("text_monitor").innerHTML = 'Swipe&larr;left to hide text';start_timer("text_monitor");}
		else {document.getElementById("text_monitor").innerHTML = 'Swipe&larr;left to hide text';clearTimeout(id_timer);start_timer("text_monitor")};		
	});
...
function block(gen, name) {
	...
	//  all_uroks_names = new Array;
		...
		urok = gen;
		glob_name = name;
		document.getElementById("nameLesson").innerHTML = gen+'.&nbsp;&nbsp;'+name.innerHTML;
		...
		if (gen==1) {			  
			document.getElementById("verb").innerHTML ='<option value="0" selected="selected">love</option>'+
		...
		if (gen==2) {
			document.getElementById("verb").innerHTML ='<option value="0"  selected="selected">questions</option>'+
		...
		if (gen==3) {
		   document.getElementById("verb").innerHTML ='<option value="0"  selected="selected"></option>';
		   $("#selector").css({"display":"none"});					
		}	
		...
		if (gen==7) {
		  document.getElementById("verb").innerHTML ='<option value="0"  selected="selected">buy</option>'+
		...
		if (gen==8) {
		  document.getElementById("verb").innerHTML ='<option value="0"  selected="selected">go</option>'+
...
function start_urok() {
			messages_length = xhr.responseXML.getElementsByTagName( 'm' )[currentVerb].getElementsByTagName( "d" ).length;
			click35();
			click4();
...
function click1plus1() {
		...
		number_click = number_click +1;	
			...
			if (number_click == 1)    // если клик первый то
		{	
...
function click1() {
		...
		{	
			document.getElementById("verb_counter").innerHTML = verbAllCounter;
			message_top = getElemText(xhr.responseXML.getElementsByTagName( 'm' )[currentVerb].getElementsByTagName( first )[random_number]); // берем данные для верхней строки
			message_middle = getElemText(xhr.responseXML.getElementsByTagName( 'm' )[currentVerb].getElementsByTagName( second )[random_number]); // берем данные для средней строки
			message_bottom = getElemText(xhr.responseXML.getElementsByTagName( 'm' )[currentVerb].getElementsByTagName( third )[random_number]); // берем данные для нижней строки
				
			if (first=="a") // если язык англ. то
			{
			  data_top = '<em>'+c1+'</em><span><a href="javascript: void(0)" onclick="playSound3(message_top, currentVerb);">'+
			  message_top+'</a></span>'; // 'фраза' = верхняя строка
			...
			else 	// если язык русский то
			{
			  ...
			  data_top = '<em>'+c1+'</em><span><a href="javascript: void(0)" onclick="playSound3(mesmid, currentVerb, message_top);">'+ message_top +'</a></span>'; // 'фраза' = верхняя строка
		...
		else // если клик второй то
		{  
			if (first=="a") // если язык англ. то
			{
			  mesmid2 =  message_top+' ru';
			  data_bot = '<em>'+c3+'</em><span><a href="javascript: void(0)" onclick="playSound3(mesmid2, currentVerb, message_bottom);">'+ message_bottom +'</a></span>'; // в качестве 'перевода' = нижняя строка
			  
			  if (sound & translation_audio & sound_global) { playSound3(mesmid2, currentVerb, message_bottom); }
			}
			else 	// если язык русский то
			{
			  data_mid = '<em>'+c3+'</em><span><a href="javascript: void(0)" onclick="playSound3(message_middle, currentVerb);">'+ message_middle +'</a></span>'; // в качестве 'перевода' = средняя строка
	  ...
	  // теперь определяем нужна ли транскрипция?
	    if (trans) // если транскрипция нужна
	    {
			if (first=="b") // если (язык - русский и клик первый) то
			{
				if (number_click==1) 
				{
				  data_bot = '<em>'+c2+'</em><span></span>'; // 'транскрипция' = пустая строка 
				}
				else // если (язык - русский и клик второй) то
				{
				  data_bot = '<em>'+c2+'</em><span>'+ message_bottom +'</span>'; // 'транскрипция' = нижняя строка
				} 
			}
			else // если (язык - английский) то		 
			{
				data_mid = '<em>'+c2+'</em><span>'+ message_middle +'</span>'; // 'транскрипция' = средняя строка
			}
		 }
		 else // если транскрипция не нужна
		 {
			if (first=="a") // если (язык - английский) то		 
			{
				data_mid = '<span></span>'; // строки 'транскрипция' - нет
			}
			else // если - русский то 
			{
...
function click3() {
	   ...
	   if (trans) // если флажок транскрипция стоит
	   {
		   if (message_top == '') // если чекбокс кликнули до нажатия кнопки старт
		   {
			   if (sound & sound_global)
			   {
					 if (first=="a")
		   ...
		   else // если по чекбоксу кликнули после нажатия кнопки старт
		   {		
			   if (document.getElementById("conteiner").innerHTML != "&nbsp;")
			   {
					if (first=="b") // если язык - русский 
					{	
						document.getElementById('translation_audio').innerHTML = dinamic;
						if(number_click==1)
						{
		...
		for (var i=0; i < lengthVerbs; i++)
		{
			if (document.formVerb.verb.options[i].selected)
			  { 
				  ...
				  arrIndexes2[i]=1;
				  verbAll = verbAll + xhr.responseXML.getElementsByTagName( 'm' )[i].getElementsByTagName( "d" ).length;
...
function click4() {
		...
		for (var i=0; i <  lengthVerbs; i++)
		{
			...
			if (arrIndexes2 [i] == 1)
			{
				 ML = xhr.responseXML.getElementsByTagName( 'm' )[i].getElementsByTagName( "d" ).length; 
...
function hide_phrases(j) {
		...
		if (j!=text_visibility) {
			...
			else {document.getElementById("text_monitor").innerHTML = 'Text is hidden';};
		 }
