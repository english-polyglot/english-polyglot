/*! Phrase generator v1.2 | Copyright 2012, 2015 Mr.Poligloto | www.english-polyglot.com/generator-fraz.html */
  ...
  var x='<em id="stop_intro" class="arrow-r ui-icon-delete" onclick="disappear();i=7" title="Закрыть подсказки"></em>';

  var intro=['<span class="intro1">'+x+'Для начала работы, нажмите на стрелку внизу</span>'+dwodn,'<span class="intro1">'+x+'Переведите фразу. Нажмите стрелку снова, чтобы увидеть ответ</span>'+dwodn, '<span style="float:left;margin-top:2px;">&#9650;</span><br /><span class="intro1">'+x+'Чтобы прослушать фразу еще раз, кликните по ней</span>', '<span class="intro1" style="display:inline-block">'+x+'Таблицу глаголов можно открыть/закрыть здесь</span><span>'+riodn+'</span>', '<span class="intro1" style="text-align:center">'+x+'Здесь можно выбрать глагол</span><span style="position:absolute;left:50%">'+dwodn+'</span>', '<span>'+upodn+'</span><span class="intro1" style="margin-top:-3px;">'+x+'Другие настройки генератора фраз находятся здесь</span>', '<span style="position:absolute;left:50%">'+upodn+'</span><br /><span class="intro1" style="margin-top:-3px;">'+x+'А здесь Вы найдете остальные инструкции по работе с генератором фраз</span>'];
  var ij3='<span id="ij3" class="intras"><span id="i3">&#9658;</span><span id="j3">'+x+'Таблицу глаголов можно открыть/закрыть здесь</span></span>';
  var tabl=[,['tablica-glagolov-petrova-image.gif','tablica-glagolov-petrova-image-black.gif','table-base.gif'],,['tablica-glagola-to-be-petrova-image.gif','black-tablica-glagola-to-be-petrova-image.gif','table-be.gif'],['tablica-glagola-to-be-ing-image.gif','tablica-glagola-to-be-ing-image-black.gif','table-be.gif']]
  var tab_index=0;
  var gear='0111101011';
	...
	r(function(){
		...
		if($("::content .adsbygoogle").css("display")=='none'){document.getElementById("page1").innerHTML='<div class="info">Please turn off the Adblock on this site and refresh the page</div>';
	...
	$("#page1").on("swipeleft swiperight",function(){
		...
		clearTimeout(id_timer);start_timer("text_monitor");
		tog_counter=0;	
	});
	...
	$(window).bind( "orientationchange", function( event ) {
		  ...
		  else
		  {
			  if($(window).width()<=370||$(window).height()<=370) {
				  ...
				  if (urok<3||urok>6){$("#selector").css({"display":"initial"});}
			  }
		  }
...
function block(gen, name) {
		...
		document.getElementById("nameLesson").innerHTML = gen+'.&nbsp;&nbsp;'+name.innerHTML.match(/<aa>(.*?)<\/aa>/g);
...
function playSound3(soundf, pap, soundr) {
		...
		{	 
			 ...
			 else
			 {
				...
				document.getElementById("dummy").innerHTML=
				'<audio id="my-audio">'+
				'<source src="http://www.english-polyglot.com/mobile/ttsgateway.php?ie=utf-8&tl='+lang+'&q='+soundf+'" type="audio/mpeg" />'+
...
function getElemText(node) {
		return node.text || node.textContent || (function(node){
			...
			while (i < childrens.length)
			{
				...
				{
				  case 5:_result += arguments.callee(child);break;
				  case 4:_result += child.value;break;
				  //break; 
				}
				i++;
			 }
...
function start_timer(id) {
	id_timer=window.setTimeout(function(){document.getElementById(id).innerHTML=''},5000);
}
...
function an_switch() {
		if(i==3){fadeEffect.init('ij3',0);i++;console.log(i);appear();};//отдельный id-блок!
...
function appear () {
	...
	if(i<7){
		...
		(function(){setTimeout(function()
		{
		  ...
		  {
			int.innerHTML=intro[i];fadeEffect.init('intro',1);
		  }
