...
function button_home(){
		document.getElementById("anons_switcher").innerHTML ='<em class="arrow-r ui-icon-info"></em>';
		anons=0;
		source='';
...
function block(gen, name) {
		...
		if (gen==8) {
			source='https://www.facebook.com/poliglott/videos/255473071252665/?l=6904554144736527812';
...
function click1() {
		...
		{	
			...
			else
			{
			  ...
			  data_top = '<em>'+c1+'</em><span><a href="javascript: void(0)" onclick="playSound3(mesmid, currentVerb, message_top);">'+ message_top +'</a></span>';
		...
		{  
			if (first=="a")
			{
			  ...
			  data_bot = '<em>'+c3+'</em><span><a href="javascript: void(0)" onclick="playSound3(mesmid2, currentVerb, message_bottom);">'+ message_bottom +'</a></span>';
			  
			  if (sound & translation_audio & sound_global) { playSound3(mesmid2, currentVerb, message_bottom); }
			}
			else 	// если язык русский то
			{
			  data_mid = '<em>'+c3+'</em><span><a href="javascript: void(0)" onclick="playSound3(message_middle, currentVerb);">'+ message_middle +'</a></span>';
...
function an_switch() {
		...
		else
		{
			...
			document.getElementById("anons_switcher").innerHTML ='<em class="arrow-r ui-icon-delete"></em>';
			document.getElementById('video_player').src=source;							
		}
