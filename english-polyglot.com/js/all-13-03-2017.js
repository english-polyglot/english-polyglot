...
function flow(){
    ...
    if((newwin==undefined)||!(newwin && !newwin.closed)){	// если окно открывается впервые или уже после его закрытия, то...
		...
		if (parent.screen.width<800){ // если наш гаджет это мобила или планш, то... 
			...
			if(url.match(/htm|http/i)){ //если это отдельная страница(практика ген-ра, фрагмент фильма, доп.инфа или окно входа в соцсеть) 
				//win_url=1
...
//	console.log('win_url = '+win_url)
	...
	}else{ // Теперь. Еcли окно открывается повторно, то окно вовсе не открывается
	//console.log('повторное newwin='+newwin)	
	if(newwin) newwin.focus() // сначала выводим окно на первый план
		if(url==''){ // ... и если это вопрос теста, то меняем блок ответа(заголовок окна, рамку!! и фразы) ...  
			...
			newwin.document.getElementById('print_form').style.display='none';
				...
				if(getextension(url).match(/jpg|gif|png/i)){ // если картинка, то меняем картинку
	...
	if(win_test||win_img||win_url){ // сначала выясняем, это первый или повторный вход
		// если повторный, то ...
...
//	console.log('height = '+height)
			if ((!last_width)||(last_width!=width)||(last_height!=height)||(url.match(/http/i))||(url.match(/generator-fraz/i))){ // и не совпадают размеры окна или это вход в соцсеть, то подобный вход - первый...
...
function getextension(filename){
 return /[^&]*/.exec(/[^?]*/.exec(/[^.]+$/.exec(filename)))[0]} // оставляем всё, что после ".", а потом обрезаем всё что после "?" и после "&"
